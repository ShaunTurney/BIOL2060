...
As biologists, we often find ourselves with many measurements of the same things. These "things" might be plants or people or study sites or blood samples or any other type of thing you might study. We refer to each measurement as a "dimension". While it's great to have so much information about the things you're studying, having a large number of dimensions poses challenges for statistical analysis. One challenge is that some of the measurements will likely be strongly correlated with each other because they're measuring related properties. Correlated variables provide redundant information and, furthermore, violate the assumptions of models you may wish to use (e.g., multiple regression). Another challenge is that the variables can't be visualized together to observe patterns because the number of dimensions that can be displayed on one graph is limited. For these reasons and more, it can be difficult to identify patterns in data with many dimensions. 

Principal Component Analysis (PCA) is a powerful tool for finding patterns in data with many dimensions. PCA allows you to describe your data with new variables (called principal components) that better express the similarities and differences between the things you're measuring. From there, one possibility is to choose just a few of these new variables and discard the rest. This allows you to reduce the number of dimensions in your data without losing much information.   

This tutorial has two sections: Introduction and Assignment. In the Introduction section, I introduce the method of PCA using a case study of iris flowers. You don't need to hand anything in from the Introduction. In the Assignment section, I give instructions for your assignment, which has two parts. In the first part, you will answer comprehension questions about a PCA published in a peer-reviewed study. In the second part, you and your colleagues will collect data about yourselves and then you will analyze this data using a PCA.
...
We can take a look at the first few rows of the *iris* dataset by using the *head* function in R. The measurements are given in centimeters. 

```{r iris}
head(iris)
```

We can use the *summary* and *plot* functions to further inspect the data.
...
The *iris* dataset is a good candidate for PCA because it has multiple variables (dimensions) and the variables are correlated with each other. In the following subsections, we will use PCA to identify patterns in flower morphology.  
...
Compare your solution to the PCA biplot below. Did you find the same solution? (Your solution could be correct even if it's not the same. Notice that if you rotate the scatter plot 180 degrees (i.e., turn it upsidedown) it will have the same variance on the horizonal axis. Therefore, if your solution is rotated 180 degrees from the PCA then this is an equally valid solution.)
...
Did you find the same solution as the PCA biplot, below? (This time, there are four equivalent solutions, since the scatterplot can be rotated 180 degree in two different directions.) If not, try rotating the scatterplot so that it matches the PCA. As a side note, there is also a third axis (principal component) that is not shown in the biplot below. 
...
As you can see, the PCA is simply a rotation of the data into a new coordinate system. The new coordinate system is defined by the principal component axes. By looking at the black arrows that represent the original axes, we can see the relationship between the old and new coordinate systems. The principal components are ordered so that each one describes less variance in the data than the previous. As a result, we can focus our analysis on the first few principal components and still capture most of the variation in the data. In other words, we can reduce the dimensions of our data. 

Reducing the number of dimensions is a bit useful when we start with three dimensions, but it becomes more useful when we start with even more dimensions. With more dimensions, the principles are still the same. Like with the 3D scatterplot above, we can still understand the PCA biplot as 2D view of the data from a particular angle in multidimensional space. The only difference is that the multidimensional space has more than three dimensions, which can hurt your brain if you think too much about it!  
...
cov_matrix
```
The numbers on the diagonal are called the "eigenvalues". The eigenvalues are the variances of the data on each principal components axis. We can consider the eigenvalues to be the amount of variance in the data that is explained by each principal component. We name the principal component with the largest eigenvalue, "PC1", the principal component with the second largest eigenvalue, "PC2", and so on (when there are more than two dimensions).  
...
The "Rotation" table columns give the eigenvectors for each of the original variables. As we saw above, the eigenvectors are visualized in biplots as arrows and they describe how the axes of the original variables are oriented relative to the principal component axes.
...
For the iris data, the elbow is at the 2nd or 3rd principal component (the location of the elbow is open to interpretation). Therefore, we might discard principal component 4 (and maybe 3 as well). We already know from the *summary* function that PC1 and PC2 together explain 95.8% of the variation in the data, and P1, P2, and PC3 together explain 99.5% of the variation. The final principal component only explains an additional 0.5%.      
...
We will make our own biplot using the *plot* function that is more customizable and easier to read. We can start by plotting the PC1 and PC2 values of each iris individual as a point. These values are stored, respectively, in the first and second column of iris.pca$x.
...
Let's change the default colours. Red and green are difficult for many people with colourblindness to distinguish. Purple and yellow are more colourblindness-friendly choices. To customize the colour palette, we create a variable called "colour". 
...
Next, we'll add arrows and text to visualize the eigenvectors. This will show how the axes of the original variables are oriented relative to the principal components. As we saw in the introduction, we can think of these arrows as being a view of the of original data from a particular angle in multidimensional space. We obtain the eigenvector values from the "Rotation" table.
...
**Test your understanding:** Why are the Petal.Length and Petal.Width arrows so similar? Why is the Sepal.Length arrow pointing in a different direction from the other arrows? 
...
<details>
  ...
  Similarly, Sepal.Length is pointing perpendicularly to the other arrows because it isn't correlated strongly with the other variables. Notice also that the Sepal.Length arrow is longer than the other variables. This means that Sepal.Length explains a lot of the variance in the data and so it contributes strongly to the principal components (especially PC2). To think of it more visually, the data is spread out widely on the Sepal.Length axis so the PCA orients this axis so that we can see the spread. 
...
This assignment has two parts. In the first part, you will answer comprehension questions about a PCA reported in a peer-reviewed study. The way that the authors report their PCA will act as a guide for you in the second part. In the second part of the assignment, you will perform and report your own PCA using a dataset collected by the class. 
...
2. In Figure 2, the authors present a much simpler graph than the one I made in this tutorial. They chose not to include the data points or arrows. Instead, they only include labels that indicate the eigenvectors. If they had included the data points, how many would there be and why? Why do you think the authors decided to make their graph the way they did? Do you think it was a good decision, or would you make the graph differently?
...
1. Before the tutorial, you were asked to complete a [survey](https://forms.gle/CbbTexyC7rMu1PVVA). This survey involved completing six short tasks on the website https://www.humanbenchmark.com/. The data collected from the survey is available [here](https://docs.google.com/spreadsheets/d/1AD2fgIpdxp2Bj71GOKs12U8QvUHzCYSqYqyYFvKVXxA/edit?usp=sharing). Download the data from the Google Spreadsheet as a csv file by going to File > Download > Comma-separated values. Open the downloaded file in Excel and rename the columns with short names that don't contain spaces (e.g., change "Can you play a musical instrument?" to "musician"). If any of the values are blank or nonsensical, replace them with "NA".

2. Load the data in R and explore the data using functions like *summary*, *aggregate*, and *plot*. You don't need to include this exploration in your report, but it will help you to understand the PCA. Choose the demographic variable that you think will have the greatest effect on task performance and predict its effect. (The "demographic variables" are the ones about gender, and whether the student is a musician, athlete, or gamer.) Perform a PCA on the six task performance variables. Make a biplot of the PCA and colour the points by your chosen demographic variables. **To include in your report: ** 

    (a) Predictions: Your predictions of the effect of the demographic variable on task performance. Make specific predictions that can be evaluated with a PCA. Explain the reasoning behind your predictions. (You should make these predictions before looking at the data.) 
  
    (b) Methods: A paragraph describing the methods used to gather the data and perform the PCA. Use the paper from Part I as a guide for how to describe the PCA methods.
    
    (c) Figure: A PCA biplot, coloured by your chosen demographic variable and including a figure caption. Include the R code used to generate the PCA and the plot. 

3. Report the results and discuss what they mean. **To include in your report: ** 

    (d) Results: A short paragraph reporting the PCA results. Again, use the paper from Part I as a guide for what to include in the results.
    
    (e) Discussion: A discussion, in which you interpret the results (300-500 words). Discuss how performance on different tasks correlate to each other and suggest what may cause these correlations. Assess your predictions based on your PCA results. If your predictions were wrong, suggest why this might be. Include any other interesting insights about the data you may have gained from the PCA.


### Evaluation

For each question, I indicate below the point allocation and the criteria that will be evaluated. The performance descriptions for each criterion are given in the rubric.

Part I:

1. 1 pt; Accuracy, critical thinking, communication 

2. 1 pt; Accuracy, critical thinking, communication 

3. 0.25 pts each x 4 = 1 pt; Accuracy

Part II:

2. 

    (a) 2 pts; Critical thinking, communication.
    
    (b) 2 pts; Statistical approach, communication.
    
    (c) 2 pts; Visuals, statistical reporting.

3. 

    (d) 2 pts; Statistical reporting, accuracy, communication.
    
    (e) 2 pts; Accuracy, critical thinking, communication.

Total = 13 pts

Rubric
<br><br>
<img src="rubric.png" width="600px" />
